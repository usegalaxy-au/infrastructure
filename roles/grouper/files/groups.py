...
APPROVED_DOMAINS_FILE = 'approved_domains.json'
USERS_FILE = 'users.json'

with open(APPROVED_DOMAINS_FILE) as f:
    approved_domains = json.load(f)
...
argparser.add_argument(
    '-d', '--dryrun', action='store_const', const=True, default=False,
    help="Do a dry run. List changes, but do not act")
argparser.add_argument(
...
def get_groups():
    ...
    if res.status_code != 200:
        print(
            "ERROR: Request did not return ok: " + res.reason + ': '
            + res.text)
        return False
    ...
    for group in groups:
        group_i += 1
        sys.stdout.write(
            "Populating group: " + group['name'] + " (" + str(group_i)
            + "/" + str(len(groups)) + ")   \r")
        sys.stdout.flush()
        queryURL = (
            GALAXY_BASEURL + config.GALAXY_GROUP_EP + group['id']
    ...
    print(
        str(len(groups)) + " groups queried. Total query time: "
...
def get_users():
    ...
    print(
        str(len(ret)) + " users returned. Query took: "
        + str(timedelta(seconds=time() - start)))
    return ret
...
def identify_remove_users(groups, dummy=False):
    ...
    for group in groups.keys():
        ...
        if group['name'] not in approved_domains.keys():
            print(
                "Group '" + group['name']
                + "' not set for automatic assignment. Skipping")
            continue
        ...
        for user in group['users']:
            ...
            domain = email[1]
            if domain not in approved_domains[group['name']]:
        ...
        if added:
            ...
            if len(assigned.keys()) > 0:
                ...
                if DRYRUN:
                    ...
                    msgs.append(
                        f"{user.email} assigned to group/s: {group_list}")
        # TODO this
        # else:
...
def identify_add_users(users, groups, dummy=False):
    ...
    for user in users:
        ...
        if len(email) != 2:
            print(
                "Malformed email address for user: " + str(user)
                + ". Skipping")
            continue
        ...
        if email[1] in approved_domains_T.keys():
            ...
            for identified_group in identified_groups:
                ...
                if not in_group:
                    ...
                    if not DRYRUN and not dummy:
                        # TODO add check for result here
                        add_user(user.id, group_ids[identified_group])
...
def list_group_domains(groups):
    ...
    for group in groups.keys():
        ...
        for user in group['users']:
            ...
            if group_name not in group_domains.keys():
...
def run(users):
    ...
    if LIST_DOMAINS:
        group_domains, no_email_users, bad_email_users = list_group_domains(
            groups)
...
def main(dryrun=True, production=False, list_domains=False, add=False,
         remove=False, notify=False, all_users=False, generate=False):
    global GALAXY_BASEURL, GALAXY_API_KEY
    global DRYRUN, PRODUCTION, LIST_DOMAINS, ADD, REMOVE, NOTIFY
    global ALL_USERS, GENERATE
    DRYRUN = dryrun
...
if __name__ == "__main__":
    ...
    main(
        dryrun=args.dryrun, production=args.production, list_domains=args.list,
        add=args.add, remove=args.remove, notify=args.notify,
        all_users=args.all, generate=args.generate)
