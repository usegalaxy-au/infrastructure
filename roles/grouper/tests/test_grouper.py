...
def test_all_users_flag_forces_pass_even_when_unchanged(
    params, make_galaxy, fake_slack, domains, make_user, make_group, capsys,
):
    user = make_user('u1', 'alice@uq.edu.au')
    group = make_group('g1', 'AU Researchers', users=[])
    ...
    assert 'Users to be added to groups' in out
    assert 'alice@uq.edu.au' in out
...
def test_without_all_users_flag_unchanged_users_short_circuit(
    ...
    out = capsys.readouterr().out
    assert 'No new users detected' in out
    assert 'Users to be added to groups' not in out
