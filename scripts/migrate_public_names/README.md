...
1. Prefix all legacy purged users with double underscore - new ones are already obfuscated: `psql -f update_purged_usernames.sql`
2. Make a copy of the galaxy_user table named galaxy_user_backup: `CREATE TABLE galaxy_user_backup AS TABLE galaxy_user;`
3. Run the migration script in dry-run mode: `psql -f migrate_public_names.sql`
4. Set COMMIT mode to 1
5. Run the migration script in commit mode: `psql -f migrate_public_names.sql`
6. Copy the output indicating backup tables and store safely for future reference
7. Verify migration by running `select count(*) from migration_mapping_tmp where change_type != 'unchanged';`
